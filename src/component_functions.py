...
def common_rafter(pitch,width,eaves_overhang,ridge_thickness = 0.035):
    try:
        ...
        print("You cannot use a zero in the numerator or denominator")   
...
def hip_rect_build(length,width):
    ...
    while True:    # Gathering crucial information and storing in a dictionary.
        try:
            ...
            print("Invalid input. Enter only type numbers")
...
# This is used in menu item 3. Which is the swiss knife of carpentry component

def balustrade(length,width_material,approx_spacing):
    
    distance = length+width_material
    number_spacing = round(distance/(width_material+approx_spacing))
    final_measurement = distance/number_spacing
    display = 0
    list_num = []
    for count in range(0,number_spacing):
        list_num.append(round(display,3))
        display += final_measurement
    print(*list_num,sep = ",")
